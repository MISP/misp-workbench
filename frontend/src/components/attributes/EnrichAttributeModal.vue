...
function queryModules() {
  ...
  const selectedModules = modules.value.filter((module) => module.query);

  modulesStore
    .queryAll(selectedModules, props.attribute)
...
function enrichAttribute() {
  ...
  const objectsPromises = objects.map((object) => {
    ...
    return objectsStore.create(object).then((response) => {
  ...
  const attributesPromises = attributes.map((attribute) => {
    ...
    return attributesStore.create(attribute).then((response) => {
      emit("attribute-created", { attribute: response });
    });
  });

  const promises = [...objectsPromises, ...attributesPromises];
          ...
          <div>
            ...
            <div class="text-start m-3">
              <div class="row">
                ...
                <div class="col">
                  <div
                    v-if="modulesResponses.length"
                    class="form-check float-end"
                  >
                    <div class="form-check float-end fs-5">
                      <label>Select All Enrichments</label>
                      <input
                        id="btn-check-all-enrichments"
                        class="form-check-input"
                        type="checkbox"
                        v-model="allEnrichments"
                        @change="toggleAllEnrichments"
                      />
                    </div>
                  </div>
                </div>
