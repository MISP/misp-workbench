...
def update_attributes_from_fetched_event(
    ...
    local_event = create_attributes_from_fetched_event(
        db, local_event, new_attributes, user
    )
    ...
    for batch_start in range(0, len(updated_uuids), batch_size):
        ...
        for db_attribute in db_attributes:
            updated_attribute = updated_attributes_dict[db_attribute.uuid]
            db_attribute.timestamp = updated_attribute.timestamp
            db_attribute.category = updated_attribute.category
            db_attribute.type = updated_attribute.type
            db_attribute.value = updated_attribute.value
            ...
            db_attribute.object_relation = updated_attribute.object_relation
            db_attribute.first_seen = updated_attribute.first_seen
            db_attribute.last_seen = updated_attribute.last_seen
